...
test("config exposes template registry, npm registry, and cache path helpers", () => {
  ...
  process.env.MYMARINA_TEMPLATE_REGISTRY_URL = "https://example.com/templates.json";
  process.env.MYMARINA_NPM_REGISTRY = "https://registry.example.com/";

  assert.equal(config.getTemplateRegistryUrl(), "https://example.com/templates.json");
  assert.equal(config.getNpmRegistry(), "https://registry.example.com");
  assert.equal(config.getTemplateCachePath("/tmp/.marina-cli"), "/tmp/.marina-cli/templates");
