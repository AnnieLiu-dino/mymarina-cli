...
async function create(projectName, options = {}) {
  ...
  const templateSource = await resolveTargetTemplate(options);
...
async function resolveTargetTemplate(options) {
  // 如果有指定的local pkg模版
...
async function resolveTemplateMeta(options) {
  ...
  let templateName = options.templateName || options.template;

  if (!templateName) {
    const inquirerTool = options.prompt || prompt;

    templateName = await inquirerTool.askSelect({
      message: "Select a project template",
      choices: templateList.map(prompt.createTemplateChoice),
    });
  }
...
async function collectTemplateData(projectName, templateConfig, options) {
  ...
  const inquirerClient = options.prompt || prompt;
  ...
  for (const item of templateConfig.prompts) {
    ...
    data[item.name] = await inquirerClient.askText({
