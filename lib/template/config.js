...
function normalizeTemplateConfig(config = {}) {
  return {
    ...
    scripts: config.scripts && typeof config.scripts === "object" ? config.scripts : {},
  };
}
...
async function loadTemplateConfig(packageRoot) {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error(`Template package is missing template.config.json: ${configPath}`);
  }

  if (!fs.existsSync(templatePath)) {
    throw new Error(`Template package is missing template directory: ${templatePath}`);
  }
