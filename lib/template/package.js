...
const { resolveExecutable } = require("../utils/command");
...
function runNpmInstall(job) {
  return new Promise((resolve, reject) => {
    const child = spawn(resolveExecutable("npm"), job.args, {
      stdio: "inherit",
      cwd: job.cachePath,
    });
...
class TemplatePackage {
  ...
  async resolveVersion() {
    if (this.packageVersion === "latest") {
      this.packageVersion = await this.npmServer.getLatestVersion(
        this.packageName,
      );
    }
  ...
  async install(options = {}) {
    ...
    // spwan 将npm包安装到缓存目录，--no-save 不写入 pkg.json
    await this.runner({
      args: [
        "install",
        this.packageSpec,
